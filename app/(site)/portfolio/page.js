...
export default async function PortfolioPage() {
  const cmsWork=await getPublicRecords("portfolio");
  const portfolioWork=cmsWork.length?cmsWork.map(x=>({title:x.title,type:x.category||"Portfolio project",image:x.afterImage||x.beforeImage,problem:x.description||"",decision:x.description||"",scope:x.location||x.category||"",priorities:"",deliverables:""})):work;
  const schema = {
