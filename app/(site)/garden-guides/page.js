...
export default async function GuidesPage() {
  ...
  const cmsSlugs = new Set(cmsArticles.map((item) => item.slug));
  const normalizedCms = cmsArticles.map((item) => ({ ...item, image: item.featuredImage || null, readTime: item.readTime || "5 min" }));
  const allGuides = [...normalizedCms, ...guides.filter((item) => !cmsSlugs.has(item.slug))];
  const schema = {
  ...
  const featured = normalizedCms.find((item) => item.featured) || allGuides[0];
  return (
        ...
        <div className="container guides-hero__inner">
          ...
          <Link className="featured-guide motion-rise motion-delay-1" href={`/garden-guides/${featured.slug}`}>
            <span className="featured-guide__media">
              {featured.image && <Image src={featured.image} alt="" width={1200} height={800} priority />}
            </span>
