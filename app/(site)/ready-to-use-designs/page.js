...
export default async function PremadeDesignsPage() {
  const cmsProducts=await getPublicRecords("products");
  const cmsSlugs=new Set(cmsProducts.map(x=>x.slug));
  const plans=[...cmsProducts.map(x=>({slug:x.slug,name:x.title,image:x.image,summary:x.description,price:x.price,etsyUrl:x.etsyUrl})),...premadePlans.filter(x=>!cmsSlugs.has(x.slug))];
  const schema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Premade garden designs",
    url: absoluteUrl("/ready-to-use-designs"),
    hasPart: plans.map((plan) => ({
      "@type": "CreativeWork",
      name: plan.name,
